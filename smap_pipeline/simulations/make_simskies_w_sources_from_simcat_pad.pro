PRO MAKE_SIMSKIES_W_SOURCES_FROM_SIMCAT_PAD, res,numpix, $
   numpixy=numpixy,$
   spec=spec,ell=ell, $
   fwhm=fwhm, $
...
if KEYWORD_SET(beam_maps) or KEYWORD_SET(FWHM) or $
   KEYWORD_SET(beam_cl) or  KEYWORD_SET(beam_bl) or $
   KEYWORD_SET(beam_profile) then convolve=1
nwv=n_elements(wv)
...
WHILE ndraw_left GT 10 DO BEGIN
   ...
   FOR w=0,nwv-1 do begin 
      ...
      tempmap[rand_x[0:ndraw_above-1],rand_y[0:ndraw_above-1]]+=$
	 ALL_FLUX[w,ind_s[0:ndraw_above-1]]
...
FOR w=0,nwv-1 do begin
   ...
   IF KEYWORD_SET(convolve) THEN BEGIN
	 ...
	 print, 'convolving by '+beamfile[w]+' beam'
      ENDIF ELSE $
      IF KEYWORD_SET(beam_cl) OR KEYWORD_SET(beam_bl) THEN BEGIN
	 IF KEYWORD_SET(beam_cl) THEN BEGIN
	    ...
	    ps_psf=ONE_D_TO_TWO_D_PS(sqrt(cl_psf_in), ell_in, res, l2, l2)
	    print, 'convolving by '+beam_cl[w]+' beam'
	 ENDIF
	 IF KEYWORD_SET(beam_bl) THEN BEGIN
	    if not(file_test(beam_bl[w])) then begin
	       print, "can't find "+beam_bl[w]
	    endif
	    readcol, beam_bl[w], ell_in, bl_psf_in
	    ps_psf=ONE_D_TO_TWO_D_PS(bl_psf_in, ell_in, res, l2, l2)
	    print, 'convolving by '+beam_bl[w]+' beam'
	    ;stop
	 ENDIF
      ENDIF ELSE BEGIN
...
;	    psf=psf/TOTAL(psf)/(res/3600.*!PI/180.0)^2.
	 endelse
	 sim_im  = REAL_PART( FFT(FFT(padmap*fac/l2^2., DOUBLE=double) * $
	    ps_psf, $
	    /INV, DOUBLE=double) ) * l2^2. ;imangx * imangy
      endif else begin
	 if keyword_set(jy_beam) then begin
	    psf=psf/max(psf)
	 endif else begin 
	    psf=psf/TOTAL(psf)/(res/3600.*!PI/180.0)^2.
	 endelse
	 SMOOTHMAP,padmap,psf,SMMAP=sim_im
      endelse
      cube[w,*,*]=sim_im[0:numpix-1,0:numpixy-1]
      if KEYWORD_SET(writesourcesky) THEN $
	 fits_write, writesourcesky[w],float(sim_im[0:numpix-1,0:numpixy-1]), hd
   ENDIF ELSE BEGIN
      cube[w,*,*]=tempmap
      if KEYWORD_SET(writesourcesky) THEN $
