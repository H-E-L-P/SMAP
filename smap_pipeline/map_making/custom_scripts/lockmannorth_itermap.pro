PRO LOCKMANNORTH_itermap, DATADIR=datadir, EXNAME=exname_in, $
                          DO_JACKKNIFE=do_jackknife, $
                          BADBOLOS=badbolos, EXCLUDEMASK=excludemask, $
                          ITERPARAMS=iterparams, SHORTNAME=shortname
  COMPILE_OPT IDL2

  CREATE_ITERMAP_DEFPARAMS, badbolos_def, excludemask_def, exname_def

  IF KEYWORD_SET(exname_in) THEN exname = exname_in ELSE exname = exname_def
  ...
  IF NOT KEYWORD_SET(datadir) THEN $
     datadir = "/data/spiredaq/reprocessed/LockmanNorth_L1e/"
...
; HORIZONTAL 
     ...
     todindex = INDGEN(49) + (INDGEN(49) / 7) * 7
