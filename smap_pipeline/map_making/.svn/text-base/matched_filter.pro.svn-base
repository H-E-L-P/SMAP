...
;   July 11, 2011: have it not modify input arguments (AC)
;   16AUG2011: Handle arbitrary dims, fix broken behaviour asymmetric
;   PSFs (EC)
;-
;------------------------------------------------------------------------------
...
PRO matched_filter, psf, conf, WHITE_IN=white, $
  ...
  if n_elements(psf) eq 1 then begin
    ...
    fwhm = psf
    psf_out = exp( -d^2d/(2d*(fwhm/SQRT(8*ALOG(2)))^2d) )
  ...
  ;; amplitude. Since we are cross-correlating rather than convolving
