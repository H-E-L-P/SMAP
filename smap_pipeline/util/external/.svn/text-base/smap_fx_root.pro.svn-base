;$Id: //depot/Release/IDL_81/idl/idldir/lib/fx_root.pro#1 $
;
...
;       a univariate nonlinear function.  This differs from FX_ROOT
;       only in that it returns a failure code when it doesn't work
;
; CATEGORY:
...
;
; OPTIONAL OUTPUTS:
;       SUCCESS: 1 if a root is found, 0 otherwise
;       ERRMSG:  Error message, '' if success
; EXAMPLE:
...
function smap_fx_root, xi, func, double = double, itmax = itmax, $
                       stop = stop, tol = tol, success=success, $
                       errmsg = errmsg
  ...
  while (it lt itmax and cond ne 1) do begin
    ...
    if stop eq 0 and abs(root - x[2]) le tol then begin
        ...
        it = it + 1
    endwhile
    if it ge itmax and cond eq 0 then begin
      errmsg = 'Algorithm failed to converge within given parameters.'
      return, !VALUES.F_NAN
    endif
    success = 1b
    return, root
