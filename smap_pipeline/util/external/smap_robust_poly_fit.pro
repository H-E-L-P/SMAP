FUNCTION SMAP_ROBUST_POLY_FIT,X,Y,NDEG,YFIT,SIG, NUMIT=THIS_MANY, $
	DOUBLE=DOUBLE, VERBOSE=verbose
;+
; NAME:
;	SMAP_ROBUST_POLY_FIT
;
; PURPOSE:
;	An outlier-resistant polynomial fit.  Modified version of astrolib
;       ROBUST_POLY_FIT with more control over error messages
;
; CALLING SEQUENCE:
...
;  Settle for least-squares:
   LSQFIT = 1
   CC = POLY_FIT( U, V, NDEG, YFIT , DOUBLE=DOUBLE, STATUS=status )
   IF status NE 0 AND KEYWORD_SET(verbose) THEN BEGIN
     errmsg = "Warning in smap_robust_poly_fit: "
     CASE status OF
       1 : errmsg += " singluar array encountered"
       2 : errmsg += " small pivot element encountered"
       3 : errmsg += " NaN error estimate encountered"
       4 : errmsg += " Unknown error"
     ENDCASE
     MESSAGE,errmsg,/INF
   ENDIF 
ENDIF ELSE BEGIN
...
;  Put the leftover points in the middle segment:
   ...
   ENDFOR
;  Now fit:
   CC = POLY_FIT( R,S, NDEG, DOUBLE=DOUBLE, STATUS=status )
   IF status NE 0 AND KEYWORD_SET(verbose) THEN BEGIN
     errmsg = "Warning in smap_robust_poly_fit: "
     CASE status OF
       1 : errmsg += " singluar array encountered"
