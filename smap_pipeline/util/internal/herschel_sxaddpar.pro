Pro herschel_sxaddpar, Header, Name, Value, Comment, Location, before=before, $
...
;
; CALLING SEQUENCE:
;       HERSCHEL_SXADDPAR, Header, Name, Value, [ Comment,  Location, 
;                               /SaveComment, BEFORE =, AFTER = ,
;                               FORMAT= , /PDU]
;
; INPUTS:
...
;       Modified version of SXADDPAR that treats HIERARCH like COMMENT
;       or HISTORY
;-
...
;  already there or not.
 ...
 if (nn EQ 'HISTORY ') or (nn EQ 'COMMENT ') or (nn EQ 'HIERARCH') $
...
;
      ...
      ENDIF ELSE IF nn EQ 'HIERARCH' THEN BEGIN
         ;;Hierarch is placed immediately after the last previous
         ;; HIERARCH or before the first history, unless
         ;; overridden by before or after
         if loc EQ 'END     ' then loc = 'HIERARCH'
         iloc = where(keywrd EQ loc, nloc)
         if nloc EQ 0 then iloc = where(keywrd EQ 'HISTORY ', nloc)
         if nloc gt 0 then begin
            i = iloc[nloc-1]
            if keyword_set(after) or (loc EQ 'HIERARCH') then i = i+1 < iend 
          ...
          ENDIF ELSE BEGIN
            ...
            if nloc gt 0 then begin
               ...
               if i gt 0 then header=[header[0:i-1],newline,header[i:n-1]] $
                ...
                iloc = where(keywrd EQ 'COMMENT ' OR keywrd EQ 'HIERARCH', nloc)
                if nloc Eq 0 then iloc = where(keywrd EQ 'HISTORY ', nloc)
                if nloc GT 0 then begin
                   i = iloc[0]
                   if i gt 0 then header=[header[0:i-1],newline,header[i:n-1]] $
