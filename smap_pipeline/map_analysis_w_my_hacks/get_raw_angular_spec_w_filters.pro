FUNCTION GET_RAW_ANGULAR_SPEC_W_FILTERS,map,deltal,dl_filt,pixsize,secondmap=secondmap,lmin=lmin, $
   kpower=kpower, outfilename=outfilename, log_deltal=log_deltal
...
dk=k[0,8]-k[0,7]
ind_vsm=rebin(dist(dims[0],1), dims[0], dims[1])*dk
v_stripe_mask=map*0.+1.
if dl_filt ge dk then $
   v_stripe_mask[where(ind_vsm le dl_filt)]=0.
...
FOR i=0l,nk-1 DO $
   ...
   BooArr[i]=MEAN(v_stripe_mask[R[R[I] : R[I+1]-1]]*k[R[R[I] : R[I+1]-1]]^kpower)/(binnedell[i]^kpower)
   ENDIF
