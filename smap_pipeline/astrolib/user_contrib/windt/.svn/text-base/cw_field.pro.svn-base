; $Id: cw_field.pro,v 1.1 2005/06/17 14:16:45 conley Exp $
;
...
;; At this point, we need to look at what kind of field we have:
...
if keyword_set(state.no_return) then begin
    ...
    ;; if there's new text, then turn off tracking events; otherwise,
    ;; turn off tracking:
    widget_control,state.textid,tracking_events=new_text eq 0
endif 

if (tag_names(event,/str) eq 'WIDGET_TRACKING') then goto, finish
...
;; If the entry has been modified or <CR> was hit and the user is
;; interested in all event or Just <CR> AND <CR> was the cause of
;; update then send it

IF (State.Update NE 0 AND $
    Altered GE State.Update) $
  or (keyword_set(state.no_return) and  $
      keyword_set(new_text)) THEN BEGIN
...
FUNCTION CW_FIELD, Parent, COLUMN=Column, ROW=Row, $
           ...
           no_return=no_return,text_id=text_id
