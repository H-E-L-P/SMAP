FUNCTION SMAP_GET_ANGULAR_SPEC,mapin,deltal,secondmap=secondmap,lmin=lmin,kpower=kpower,mask1=mask1,mask2=mask2,beam=beam,fsky=fsky,mkk=mkk,ell=ell,nk=nk,callc=callc,nomask=nomask,mpi=mpi
;+
;NAME
...
;-
  ...
  IF (NOT(KEYWORD_SET(mask1)) AND NOT(KEYWORD_SET(nomask))) THEN begin
     mask1=double(finite(map)) 
     mask2=mask1
  ENDIF

  IF (KEYWORD_SET(mask1) AND NOT(KEYWORD_SET(mask2))) THEN $
     mask2=mask1
  
  IF (KEYWORD_SET(fsky))THEN begin
     mask1=0
     mask2=0
  ENDIF
  ...
  IF KEYWORD_SET(mask1) THEN BEGIN
     masktouse1=dblarr(newdim,newdim)
     masktouse1[posx_b:posx_b+dims[0]-1,posy_b:posy_b+dims[1]-1]=mask1
     masktouse2=dblarr(newdim,newdim)
     masktouse2[posx_b:posx_b+dims[0]-1,posy_b:posy_b+dims[1]-1]=mask2
  ENDIF
  ...
  IF KEYWORD_SET(mask1) THEN BEGIN
     maptouse*=masktouse1
     IF KEYWORD_SET(secondmap) THEN secondmaptouse*=masktouse2
  ENDIF


  Ck=GET_ANGULAR_SPEC(maptouse,deltal,pixsize,secondmap=secondmaptouse,lmin=lmin,kpower=kpower,mask1=masktouse1,mask2=masktouse2,beam=beam,fsky=fsky,mkk=mkk,ell=ell,nk=nk,callc=callc,mpi=mpi)
