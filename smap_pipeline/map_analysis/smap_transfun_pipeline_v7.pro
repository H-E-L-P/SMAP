FUNCTION SMAP_TRANSFUN_PIPELINE_V7, conffile,$
				dirmap, mapname, $
			        map_template=map_template, EXNAMEBASE=exnamebase, $ 
				NOISE=noise,FKNEE=fknee,$
				WRITETODS=writetods,$
				SAVEMAPDIR=savemapdir
;+
;NAME
; SMAP_TRANSFUN_PIPELINE_V6
;PURPOSE
; "Observe" simulated sky maps, and then make maps the SMAP mapmaker. 
...
IF info.exname       NE "" THEN exname       = info.exname else exname=''
...
if keyword_set(map_template) then begin
   ...
   nxpix=intarr(nmaps)
   nypix=intarr(nmaps)
   pixscale=dblarr(nmaps)
   FOR i=0,n_elements(map_template)-1 DO BEGIN
...
;DO THIS FOR ONE MAP AT A TIME? RESPECTING the no250 FLAG, ETC?
mapon=dblarr(3)
if not(keyword_set(no250)) then mapon[0]=1
if not(keyword_set(no350)) then mapon[1]=1
if not(keyword_set(no500)) then mapon[2]=1
nmap=total(mapon)
if nmap eq 3 then $
   oneband=0 $ 
   else if nmap eq 1 then $ 
   oneband=1+where(mapon eq 1) $
   else begin
   print, 'you must have 1 or 3 maps, not 2!!'
endelse

simmaphead=headfits(dirmap+mapname)
...
maps=ptrarr(nmap)
mapfiles=strarr(nmap)
if nmap eq 3 then begin
   ;IS IT RIGHT TO ASSUME THE SIMMAPS ARE ALL THE SAME SIZE?
   ...
   mapstruct500.astrometry.crval=[crvalx[0],crvaly[0]]
   ...
   iband=where(mapon eq 1) 
   mapstruct=get_smap_mapstruct(npixx=skypix_side_x,npixy=skypix_side_y,band=bands[iband],/noerr,/noexp,/nomask)
   mapstruct.astrometry.cd=identity(2)
   mapstruct.astrometry.crval=[crvalx[0],crvaly[0]]
   mapstruct.astrometry.cdelt=[-1.*pixsize_degrees,pixsize_degrees]
   mapstruct.astrometry.crpix=[crpix_x,crpix_y]
   maps[0]=ptr_new(temporary(mapstruct))
endelse

;HERE I AM READING THE TODS FOR THE SOLUTON.  
;by providing datadir, I am scanning
;ALSO ENABLE oneband=oneband flag
doptrs=1
...
exnamebasewnum=exnamebase+exname
;stop
SMAP_TRANSFUN_MAKEMAP_V6, datadirbase, dataname, dataversion, $
   dirmap,mapname,maps,tods,mapparam, $
