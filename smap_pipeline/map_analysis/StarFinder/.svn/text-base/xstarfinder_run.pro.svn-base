; $Id: xstarfinder_run, v 1.3 Aug 2006 e.d. $
;
;+
; NAME:
;   XSTARFINDER_RUN
...
;         deblend = (deblend eq 1) and 1B
          ...
          if  str_threshold eq ''  then $
             ...
             starfinder, *(*data).image, *(*data).psf, $
                   ...
                   NO_SLANT = no_slant, MIN_DISTANCE = mindist, DEBLEND = deblend, $
                   N_ITER = niter, /SILENT, X_INPUT = x_input, Y_INPUT = y_input, F_INPUT = f_input, $
...
;   deblend = cw_bgroup(f_base, 'Apply deblender', /NONEXCLUSIVE)
