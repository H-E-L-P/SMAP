...
two_halo=1
clean_skies=2
allmap_pixsize=15.;24.;0.;50.
; first and last index of the simulated sky to run on
startnum=0;21;44;1;64;56;0;78;28;77;0;27;0
endnum=0;24;50;99;39;2

if clean_skies ne 0 then cs_suffix='_cs' else cs_suffix=''
if clean_skies eq 2 then cs_suffix='_act' 
...
noise=.00001*[4913503.6,3236836.4, 2219022.1]/[1.1619,1.5916,1.46926];Jy/sr/sqrt(hits);=[0.04313,0.04997,0.07763];Jy/beam/sqrt(hits)
...
map_template=dirtemplate+field_ids[field_num]+'_v4_itermap_'+exname+mapname[0:2]+'.fits'
...
exnamebase=strcompress(act_prefix+'_'+string(skypix_pixsize,format='(i10)')+'x'+ $
   string(sidex,format='(i10)')+'x'+string(sidey,format='(i10)')+$
   cs_suffix+$
   allmap_suffix+$
...
for it=startnum,its-1 do begin
   filemapname=strcompress('clean_sky_'+$
      string(skypix_pixsize,format='(i10)')+'x'+ $
      string(sidex,format='(i10)')+'x'+string(sidey,format='(i10)')+'_'+$
   ...
   test=SMAP_TRANSFUN_PIPELINE_V4(map_template,field_ids[field_num], $ 
      ...
      USE_MAPS=1, DIRMAP=dirsim, MAPNAME=filemapname, $
      USEFITS=1)
