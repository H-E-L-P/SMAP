pro aorstack_cosmos_corrected
  
;to perform stacking on the COSMOS individual aor maps, once they
;astrometry corrected. This is to check the corrections. All of them 
;should have offsets consistent with 0 now. 

mapdir = !SMAP_MAPS
astromoffsetsdir = "/data/spiredaq/astrometry/"
dataversion = 'L1e'
outfile = astromoffsetsdir+'COSMOS_'+dataversion+'_corrected.dat'
...
FOR b=0,nbands-1 DO BEGIN
    ...
    maps = FILE_SEARCH(mapdir+mappre+exname+"_*_"+bands[b]+mapsuf, COUNT=nmaps)
    ...
    FOR m=0,nmaps-1 DO BEGIN
        ...
        fchar = STRLEN(mapdir+mappre+exname+'_')
        ...
        IF b EQ 0 AND m EQ 0 THEN OPENW, lun, outfile, /GET_LUN, WIDTH=150
        IF b eq 0 AND m EQ 0 THEN PRINTF, lun, '# Band       obsID       obsName    offsetEast[arcsec]  offsetNorth[arcsec]  RA_tanpoint[deg]  Dec_tanpoint[deg]'
        printf,lun,  bands[b], '     ', aorname, '     ',shortname, $ 
               shift[0], -1.*shift[1], ratanp, dectanp
    ENDFOR
